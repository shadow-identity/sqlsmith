...
import { SqlMerger } from './sql-merger.js';
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
/**
 * Main merge command implementation
 */
export const mergeCommand = async (inputPath, options) => {
    ...
    if (!validDialects.includes(options.dialect)) {
        throw new Error(`Invalid dialect: ${options.dialect}. Must be one of: ${validDialects.join(', ')}`);
    }
    ...
    if (options.quiet) {
        ...
        try {
            const sqlFiles = merger.parseSqlFile(resolvedInput, options.dialect, {
 ...
 */
export const infoCommand = async (inputPath, options) => {
    if (!options.quiet) {
    ...
    try {
        ...
        if (!options.quiet) {
            ...
            console.log(`📊 Found ${sqlFiles.length} SQL files with valid dependencies`);
        }
        ...
        if (errorMessage.includes('Circular dependencies detected')) {
            console.log('❌ Circular dependencies detected - cannot determine safe execution order');
            // Still try to show the dependency graph visualization
            try {
                const files = [];
                const filePaths = merger.findSqlFiles(resolvedInput);
                for (const filePath of filePaths) {
                    files.push(merger.parseSingleFile(filePath, options.dialect, {}));
                }
 ...
 */
export const validateCommand = async (inputPath, options) => {
    if (!options.quiet) {
    ...
    try {
        ...
        for (const filePath of filePaths) {
            try {
                const file = merger.parseSingleFile(filePath, options.dialect, {});
                files.push(file);
                validFiles++;
                if (!options.quiet) {
 ...
 */
export const validateInputDirectory = (inputPath) => {
